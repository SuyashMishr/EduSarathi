...
class GeminiService:
    ...
    def _ensure_pdf_data_extracted(self):
        ...
        try:
            # Check if Physics 11th PDF data exists (check both English and Hindi versions)
            physics_11_en_path = Path(self.config.NCERT_DATA_DIR) / "grade_11" / "physics" / "chapters_from_pdf_english.json"
            physics_11_hi_path = Path(self.config.NCERT_DATA_DIR) / "grade_11" / "physics" / "chapters_from_pdf_hindi.json"
